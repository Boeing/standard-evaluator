...
class BoreholeMultiFiLo(BoreholeMultiFiBase):
    ...
    def _evaluate(self, sites: pd.DataFrame) -> None:
        """Call to the Borehole multifidelity low function.

        Args:
            sites: The dataframe containing input values, updated with responses.
        """
        num_part = 5.0 * sites.trans_upper * (sites.pot_upper - sites.pot_lower)
