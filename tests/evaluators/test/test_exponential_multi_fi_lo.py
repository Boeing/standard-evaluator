"""Testing of the ExponentialMultiFiLo class"""

import pytest
import pandas as pd
from standard_evaluator.evaluators.test import ExponentialMultiFiLo


def test_exponential_multi_fi_lo():
    """Test the ExponentialMultiFiHi test function"""
    # Instantiate the test function
    test_func = ExponentialMultiFiLo()
    ...
    initial_guess.loc[1, "x1"] = 0.2378
    initial_guess.loc[1, "x2"] = 0.5634
    # Evaluate the initial guess
    test_func(initial_guess)
    # Check some specific responses
    assert test_func.inputs == ["x1", "x2"]
    assert test_func.outputs == ["f"]
    assert initial_guess.iloc[0].f == pytest.approx(9.4211941)
    assert initial_guess.iloc[1].f == pytest.approx(8.0124339)


def test_create_opt_problem():
    # Create an instance of the class that contains the _create_opt_problem method
    opt_problem = ExponentialMultiFiLo()._create_opt_problem()
...
literature on computer experiments."""
    ...
    assert initial_guess_df.iloc[0].f == pytest.approx(9.421194000910116)
