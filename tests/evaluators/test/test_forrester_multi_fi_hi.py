"""Testing of the ForresterMultiFi_Hi class"""
...
def test_forrester_multi_fi_hi():
    """Test the ForresterMultiFi_Hi test function"""
    # Instantiate the test function
    test_func = ForresterMultiFiHi()
...
def test_forrester_multi_fi_hi_kwargs():
    "Test the modified values of A, B & C for ForresterMultiFiHi test function"
    ...
    options = ForresterMultiFiOptions(A=0.4, B=2.0, C=-3.3)
    test_func = ForresterMultiFiHi(options=options)
    ...
    initial_guess.loc[1, "x"] = 0.2542
    # Evaluate the initial guess
    test_func(initial_guess)
    # Check some specific responses
    assert initial_guess.iloc[0].f == pytest.approx(-6.018327)
    assert initial_guess.iloc[1].f == pytest.approx(-0.183320)
    # Test that the options values are accessible
    assert test_func.lookup_option_value("A") == pytest.approx(0.4)
    assert test_func.lookup_option_value("B") == pytest.approx(2.0)
    assert test_func.lookup_option_value("C") == pytest.approx(-3.3)


def test_create_opt_problem():
    # Create an instance of the class that contains the _create_opt_problem method
    opt_problem = ForresterMultiFiHi()._create_opt_problem()
    ...
    expected_description = """Multifidelity analysis occurs when different analysis or simulation codes predict the same
...
𝑦ℎ=𝑍𝜌𝑦𝑙+𝑍𝑑"""
    ...
    from standard_evaluator.evaluators.test.forrester_multi_fi_base import ForresterMultiFiOptions
    options = ForresterMultiFiOptions(A=0.3, B=4.0, C=-1.3)
    test_func = ForresterMultiFiHi(options=options)
    ...
    assert initial_guess_df.iloc[0].f == pytest.approx(-6.018326515965059)
    # Test that the options values are accessible
    assert test_func.lookup_option_value("A") == pytest.approx(0.3)
    assert test_func.lookup_option_value("B") == pytest.approx(4.0)
    assert test_func.lookup_option_value("C") == pytest.approx(-1.3)


def test_forrester_multi_fi_hi_options():
    """Test the options functionality for ForresterMultiFiHi"""
    from standard_evaluator.evaluators.test.forrester_multi_fi_base import ForresterMultiFiOptions
    ...
    required_opts = ForresterMultiFiHi.required_options()
    ...
    option_names = ForresterMultiFiHi.required_options_names()
    assert option_names == {"A", "B", "C"}
    
    # Test custom options
    custom_options = ForresterMultiFiOptions(A=1.5, B=3.5, C=0.5)
    test_func_custom = ForresterMultiFiHi(options=custom_options)
...
def test_forrester_multi_fi_hi_options_validation():
    """Test that options validation works correctly"""
    from standard_evaluator.evaluators.test.forrester_multi_fi_base import ForresterMultiFiOptions
    
    # Test valid options
    valid_options = ForresterMultiFiOptions(A=1.0, B=2.0, C=3.0)
    test_func = ForresterMultiFiHi(options=valid_options)
