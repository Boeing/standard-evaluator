"""Testing of the BoreholeMultiFiHi class"""

import pytest
import pandas as pd
import numpy as np
from standard_evaluator.evaluators.test import BoreholeMultiFiHi


def test_borehole_multi_fi_hi():
    """Test the BoreholeMultiFiHi test function"""
    # Instantiate the test function
    test_func = BoreholeMultiFiHi()
    ...
    initial_guess.loc[1, "rad_borehole"] = 0.0789
    initial_guess.loc[1, "rad_influence"] = 5555.78
    initial_guess.loc[1, "trans_upper"] = 67934.12
    initial_guess.loc[1, "pot_upper"] = 1001.98
    initial_guess.loc[1, "trans_lower"] = 78.345
    initial_guess.loc[1, "pot_lower"] = 789.456
    initial_guess.loc[1, "len_borehole"] = 1458.55
    initial_guess.loc[1, "hyd_con_borehole"] = 11666.89
...
def test_create_opt_problem():
    ...
    opt_problem = BoreholeMultiFiHi()._create_opt_problem()
    ...
    expected_description = """The Borehole function models water flow through a borehole. Its simplicity and quick evaluation makes it a commonly used function for testing a wide variety of methods in computer experiments. The response is water flow rate in :math:`m^3/yr`.
...
====================================================  ================"""
    ...
    assert initial_guess_df.iloc[0].water_flow_rate == pytest.approx(86.210152)
