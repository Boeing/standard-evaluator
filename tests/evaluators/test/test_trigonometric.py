...
import pytest
import pandas as pd
from standard_evaluator.evaluators.test import Trigonometric


def test_trigonometric():
    """Test the Trigonometric test function"""
    # Instantiate the test function
    test_func = Trigonometric()
    ...
    assert initial_guess.iloc[0].f == pytest.approx(0.16357346659335475)
    assert initial_guess.iloc[1].f == pytest.approx(2.212772251402149)


def test_create_opt_problem():
    # Create an instance of the class that contains the _create_opt_problem method
    opt_problem = Trigonometric()._create_opt_problem()
    ...
    expected_initial_guess = (0.5, 0.5)
    ...
    assert initial_guess_df.iloc[0].f == pytest.approx(0.16357346659335475)
