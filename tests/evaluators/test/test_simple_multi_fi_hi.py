"""Testing of the SimpleMultiFi_Hi class"""

import pytest
import pandas as pd
import math
import numpy as np
from standard_evaluator.evaluators.test import SimpleMultiFiHi


def test_simplemultifi_hi():
    """Test the SimpleMultiFi_Hi test function"""
    # Instantiate the test function
    test_func = SimpleMultiFiHi()
    ...
    assert initial_guess.iloc[0].c1 == pytest.approx(2.55628)
    assert initial_guess.iloc[0].c2 == pytest.approx(-0.99999999)
    assert initial_guess.iloc[1].f == pytest.approx(-0.23681662964894892)
    assert initial_guess.iloc[1].c1 == pytest.approx(17.39006517575012)
    assert initial_guess.iloc[1].c2 == pytest.approx(-0.9201533925731991)


def test_simplemultifi_hi_opt():
    "Test the optimal solution of the OptlibTest test function"
    # Instantiate the test function
    test_func = SimpleMultiFiHi()
    # Get the initial guess
    ...
    # Check some specific responses
    assert known_sol.iloc[0].f == pytest.approx(-0.942780004147445)
    assert known_sol.iloc[0].c1 == pytest.approx(-4.4339580451691063e-07)
    assert known_sol.iloc[0].c2 == pytest.approx(-0.8058778822145461)


def test_create_opt_problem():
    # Create an instance of the class that contains the _create_opt_problem method
    opt_problem = SimpleMultiFiHi()._create_opt_problem()
...
$$"""
    ...
    assert initial_guess_df.iloc[0].c1 == pytest.approx(2.556280200150308)
    assert initial_guess_df.iloc[0].c2 == pytest.approx(-0.9999999999999999)
